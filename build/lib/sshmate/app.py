...
class ConfirmDelete(ModalScreen):
    ...
    def on_key(self, event):
        if event.key in ("left", "right"):
            ...
            if focused and focused.id == "confirm":
...
class SSHMateApp(App):
    BINDINGS = [
        ...
        Binding("shift+up,K", "move_up", "Move Up"),
        Binding("shift+down,J", "move_down", "Move Down"),
        Binding("q", "quit", "Quit"),
    ]
    ...
    def action_move_up(self):
        lv = self.query_one("#connection-list", ListView)
        index = lv.index
        if index is None or index == 0 or index >= len(self.connections):
            return
    ...
    def action_move_down(self):
        lv = self.query_one("#connection-list", ListView)
        index = lv.index
        if index is None or index >= len(self.connections) - 1:
            return
